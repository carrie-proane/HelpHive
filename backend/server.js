  ...
  for (const [languageIndex, language] of languages.entries()) {
    ...
    for (const profile of profiles) {
      try {
        ...
        const calibratedConfidence = calibrateOcrConfidence(averageConfidence, text);

        if (text) {
          const candidateLowConfidenceWords =
            calibratedConfidence >= OCR_CONFIDENCE_THRESHOLD ? [] : lowConfidenceWords;

          candidates.push({
            text,
            averageConfidence: calibratedConfidence,
